...
TEST_NAME="TestPavProjectInfoCmd"
...
test01()
{
  ...
  CLIENT project info valid_all > $TEMP_LOG
  flag=0
  is_empty $(grep "name" $TEMP_LOG | grep "valid_all")
  flag=$[ $flag | $? ]
...
test02()
{
  ...
  # info an invalid project
  project_name="project_$(echo "$RANDOM")"
  CLIENT project info $project_name > $TEMP_LOG 2>&1
